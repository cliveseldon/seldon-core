...
def run_model(model_name):
    with open(model_name, 'r') as stream:
        ...
        run(f"kubectl rollout status deploy/$(kubectl get deploy -l seldon-deployment-id={metaName} -o jsonpath='{{.items[0].metadata.name}}')", shell=True)
        for i in range(60):
            ret = Popen(f"kubectl get sdep {metaName} -o jsonpath='{{.status.state}}'", shell=True, stdout=subprocess.PIPE)
...
def run_test(vegeta_cfg,vegeta_job,wait_time):
    with open(vegeta_job, 'r') as stream:
        ...
        run(f"kubectl wait --for=condition=complete --timeout={wait_time} job/tf-load-test", shell=True)
        ret = Popen(f"kubectl logs $(kubectl get pods -l job-name={metaName} -o  jsonpath='{{.items[0].metadata.name}}')", shell=True, stdout=subprocess.PIPE)
